    ...
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center animate-in fade-in zoom-in duration-200 p-4">
      <div class="glass-panel w-full max-w-md p-6 relative shadow-2xl border border-white/10 rounded-2xl">
        ...
        <div class="flex gap-3 mt-8">
          <button (click)="onCancel()" class="flex-1 px-4 py-2.5 rounded-xl border border-white/10 hover:bg-white/5 transition-all text-sm font-medium">
            {{ cancelText }}
          </button>
          <button (click)="onConfirm()" class="flex-1 px-4 py-2.5 rounded-xl bg-red-500/80 hover:bg-red-500 text-white transition-all text-sm font-bold shadow-lg shadow-red-500/20">
            {{ confirmText }}
          </button>
        </div>
      </div>
