...
@Component({
  ...
  template: `
    <div class="space-y-6 animate-in fade-in duration-500">
      <!-- Header -->
      <div
        class="flex justify-between items-center bg-white/5 border border-brand-border rounded-2xl p-6 glass-panel"
      >
        <div>
          <h1 class="text-2xl font-bold">Contacts</h1>
          <p class="text-brand-secondary text-sm mt-1">
            Manage your customer relationships and leads
          </p>
        </div>
        <app-button variant="premium" (clicked)="openCreateModal()">
      ...
      @if (isModalOpen) {
        <div
          class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center animate-in fade-in zoom-in duration-200"
        >
      ...
      <div class="glass-panel overflow-hidden">
        @if (isLoading$ | async) {
          <div class="flex flex-col items-center justify-center p-20 space-y-4">
            ...
            ></div>
            <p class="text-brand-secondary animate-pulse">
              Retrieving contacts...
            </p>
          </div>
        } @else {
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="border-b border-brand-border bg-white/5">
                  <th
                    class="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-brand-secondary"
                  >
                    Contact
                  </th>
                  <th
                  ...
                  <th
                    class="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-brand-secondary text-right"
                  >
                    Actions
                  </th>
                </tr>
                  ...
                  <tr class="hover:bg-white/[0.02] transition-colors group">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
                        <div
                          class="w-10 h-10 rounded-full bg-gradient-premium flex items-center justify-center text-sm font-bold"
                        >
